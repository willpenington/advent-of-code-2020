defmodule Day02Test do
  ...
  test "parse string into rule" do
    assert Day02.extract_record("1-3 a: abcde") == %Record{
             from: 1,
             to: 3,
             char: "a",
             password: "abcde"
    ...
    assert Day02.extract_record("1-3 b: cdefg") == %Record{
             from: 1,
             to: 3,
             char: "b",
             password: "cdefg"
    ...
    assert Day02.extract_record("2-9 c: ccccccccc") == %Record{
             from: 2,
             to: 9,
             char: "c",
             password: "ccccccccc"
  ...
  test "valid password matches by count rule" do
    ...
    assert Day02.password_matches_by_count(%Record{
             from: 2,
             to: 9,
             char: "c",
             password: "ccccccccc"
  ...
  test "invalid password fails match by count" do
    assert Day02.password_matches_by_count(%Record{from: 1, to: 3, char: "b", password: "cdefg"}) ==
             false
  ...
  test "valid password matches by possition rule" do
    assert Day02.password_matches_by_position(%Record{
             from: 1,
             to: 3,
             char: "a",
             password: "abcde"
